...
def map_states_from_wins(hmm_wins):
    ...
    mapping = {good_hmm: 'good', mediocre_hmm: 'mediocre', bad_hmm: 'bad'}
    return mapping

# def calculate_accuracy(hmm_wins, state_mapping, alpha=0.05):
#     """
#     Calculate accuracy based on forward health progression.
#     Penalize for reverse transitions (e.g., bad -> good).
#     """
#     state_order = {'good': 0, 'mediocre': 1, 'bad': 2}
#     states_sequence = [state_order[state_mapping[winner]] for winner in hmm_wins]

#     accuracy = 1.0
#     for i in range(1, len(states_sequence)):
#         if states_sequence[i] < states_sequence[i-1]:
#             accuracy -= alpha  # penalize for each reverse jump

#     return max(0.0, accuracy)  # accuracy cannot be negative

def calculate_accuracy(hmm_wins, state_mapping, alpha=0.05):
    """
    Calculate accuracy based on forward health progression.
    Penalize for reverse transitions (e.g., bad -> good).
    Skip unknown HMMs safely.
    """
    state_order = {'good': 0, 'mediocre': 1, 'bad': 2}
    states_sequence = []

    for winner in hmm_wins:
        if winner in state_mapping:
            states_sequence.append(state_order[state_mapping[winner]])
        else:
            # If winner not mapped, skip it
            continue

    accuracy = 1.0
    for i in range(1, len(states_sequence)):
        if states_sequence[i] < states_sequence[i-1]:
            accuracy -= alpha  # Penalize for reverse jumps

    return max(0.0, accuracy)  # Accuracy can't go negative


def main():
    data_dir = './data'
    hmms_dir = './results/top_hmms_kl/'  # Using HMMs selected via KL divergence
    ...
    # Split datasets
    train_sequences = sequences_2d[:10]  # First 10 files for training
    test_sequences = sequences_2d[10:]   # Remaining files for testing

    train_accuracies = []
    test_accuracies = []

    print("\n--- Training Data Diagnostics ---\n")
    for idx, sequences in enumerate(train_sequences):
        hmm_wins = evaluate_hmm_wins(hmms, sequences)
        mapping = map_states_from_wins(hmm_wins)
        accuracy = calculate_accuracy(hmm_wins, mapping)

        train_accuracies.append(accuracy)

        print(f"DB{idx+1}:")
        print(f"HMM Wins Sequence: {hmm_wins}")
        print(f"State Mapping: {mapping}")
        print(f"Accuracy: {accuracy:.2f}\n")

    print("\n--- Testing Data Diagnostics ---\n")
    for idx, sequences in enumerate(test_sequences):
        hmm_wins = evaluate_hmm_wins(hmms, sequences)
        mapping = map_states_from_wins(hmm_wins)
        accuracy = calculate_accuracy(hmm_wins, mapping)
